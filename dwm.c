...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
        ...
        if ( scheme_num == SchemeSel ){
            if (occ & 1 << i) {
                drw_text(drw, x, 0, w, bh, lrpad / 2, tagsymbols[1], urg & 1 << i);
            } else {
                drw_text(drw, x, 0, w, bh, lrpad / 2, tagsymbols[2], urg & 1 << i);
            }
			//drw_rect(drw, x + boxs, boxs, w-2, 1,
